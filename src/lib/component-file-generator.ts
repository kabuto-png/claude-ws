...
import { getAgentFactoryDir } from './agent-factory-dir';
...
export interface ComponentFileExistsError extends Error {
  code: 'COMPONENT_EXISTS';
  path: string;
}

/**
 * Generate component file(s) on disk for Agent Factory components
 ...
 *
 * @throws {ComponentFileExistsError} If component file already exists
 */
export async function generateComponentFile(options: GenerateComponentFileOptions): Promise<void> {
  ...
  if (existsSync(targetPath)) {
    const error = new Error(`Component already exists at ${targetPath}`) as ComponentFileExistsError;
...
/**
 * Get the file path for a component (without creating it)
 */
export function getComponentPath(type: 'skill' | 'command' | 'agent', name: string): string {
  const slug = toKebabCase(name);
  const agentFactoryDir = getAgentFactoryDir();
 ...
 */
export function componentExists(type: 'skill' | 'command' | 'agent', name: string): boolean {
  const path = getComponentPath(type, name);
  return existsSync(path);
}

/**
 * Generate YAML frontmatter for component file
 */
